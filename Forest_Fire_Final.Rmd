...
```{r fig.height=4, fig.width=6, tidy=TRUE, tidy.opts=list(width.cutoff=75)}
plot(jitter(forest_fire$X, factor=2.5), jitter(forest_fire$Y, factor=2.5), 
     col=forest_fire$fire_size, cex=forest_fire$dotsize)
...
```{r fig.height=3, fig.width=4, tidy=TRUE, tidy.opts=list(width.cutoff=75)}
...
```{r fig.height=4, fig.width=6, tidy=TRUE, tidy.opts=list(width.cutoff=50)}
ggplot(forest_fire, 
...
```{r fig.height=3, fig.width=4, tidy=TRUE, tidy.opts=list(width.cutoff=50)}
...
ggplot(areadm, 
       aes(Group.2, Group.1)) + geom_tile(aes(fill = x)) + scale_colour_gradient2()+labs(x="month",  
       y="day", title = "Heat map by month and day")
