...
First, we load required libraries and the forest fires data set.
...
We will elaborate on the justifications for these transformations later on.
...
We begin with a scatterplot matrix. This is helpful for getting a high-level overview of the relationships between our variables and can draw our attention to important features we want to investigate further. Note that to plot all the 13 variables in one matrix reduces legibility, so we will just visualize a subset of the variables here.
...
Area appears to be right skewed. To normalize the data, we took the log of area above in our transformations section.
...
We noticed via our bivariate analysis that month is displaying secondary effects with DMC, DC, and RH. We don't have enough information to say for certain which way the causation is pointing. Our hypothesis would be that certain months of the year cause changes in variables like the relative humidity or the organic matter. This general type of pattern can be visualized in the following influence diagram. 

```{r}

# Trying to make an influence diagram but not working

# net <- HydeNetwork(~ area | month*DMC
#                    + DMC | month)
# plot(net)
```

Below are several examples of secondary effects we noticed. In particular, month seems to be correlated with DC & DMC.
...
plot(forest_fire$month, forest_fire$DMC)
