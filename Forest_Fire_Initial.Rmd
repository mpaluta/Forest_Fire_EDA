...
forest_fire$fire_size <- cut(forest_fire$area, breaks=c(-Inf,0.01, 12.85, Inf), labels=c('Low','Medium','High'))
...
```{r}
ggplot(forest_fire, aes(month, ..count..))+geom_bar(aes(fill=sorted_day)) + facet_grid (fire_size~.) + labs(x="month") + labs( y="Count of fires")
